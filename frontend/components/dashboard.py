...
def render_stats_widgets():
    ...
    try:
        ...
        with col3:
            st.metric("Akurasi Model", f"{acc:.1%}", help="Performa Model Random Forest")
        ...
        with chart_col1:
            ...
            df_score = data.groupby(['Satisfaction_Score', 'Is_Churn']).size().reset_index(name='Jumlah')
            ...
            fig_bar = px.bar(
                    ...
                    color="Is_Churn",
                    barmode="group",
                    category_orders={"Satisfaction_Score": ["1", "2", "3", "4", "5"]}, # Mengunci urutan skor
                    color_discrete_map={1: '#EF4444', 0: '#10B981'}
                )
            fig_bar.update_layout(xaxis_title="Skor Kepuasan", yaxis_title="Jumlah Pelanggan")

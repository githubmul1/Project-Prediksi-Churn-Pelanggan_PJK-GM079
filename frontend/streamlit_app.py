...
if menu == "Dashboard":
    ...
    with st.form("form_prediksi"):
        ...
        with col1:
            age = st.number_input("Umur", 18, 100, value=None, placeholder="Masukkan umur pelanggan")
            sub_months = st.number_input("Lama Berlangganan (Bulan)", 1, 72, value=None, placeholder="Masukkan lama berlangganan")
            monthly_logins = st.number_input("Rata-rata Login Bulanan", 0, 50, value=None, placeholder="Masukkan rata-rata login bulanan")
            last_purchase = st.number_input("Hari Sejak Pembelian Terakhir", 0, 365, value=None, placeholder="Masukkan hari sejak pembelian terakhir")
            usage_time = st.number_input("Waktu Penggunaan App (Menit)", 0, 5000, value=None, placeholder="Masukkan waktu penggunaan app")
            monthly_spend = st.number_input("Pengeluaran Bulanan (USD)", 0, 1000, value=None, placeholder="Masukkan pengeluaran bulanan")
            discount_pct = st.number_input("Persentase Diskon yang Digunakan", 0, 100, value=None, placeholder="Masukkan persentase diskon yang digunakan")
            support_calls = st.number_input("Jumlah Panggilan Dukungan", 0, 20, value=None, placeholder="Masukkan jumlah panggilan dukungan")
            satisfaction = st.slider("Skor Kepuasan (1-5)", 1, 5, value=1, help="Pilih Tingkat kepuasan")
            contract_type = st.selectbox("Jenis Kontrak", ["Monthly", "Quarterly", "Annual"], index=None, placeholder="Pilih jenis kontrak")
    ...
    if submit_button:
        required_fields = [age, sub_months, monthly_logins, last_purchase, usage_time, monthly_spend,
                           discount_pct, support_calls, satisfaction, contract_type]
